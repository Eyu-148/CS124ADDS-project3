...
private:
    ...
    // the function should return the depth to search the value and the message if ht value
    // is found
    int find(const Comparable &c, BinaryNode* n, int& dep) const {
        if (n == nullptr) {
            // Reached a dead end. Value not in tree.
            std::cout << "Value is Not in Tree" << std::endl;
            // No node found, back to the last level
            return dep - 1;
        }
        if (c < n->value) {
            // Value is less than current node. Go to node's left child.
            int currentDep = dep + 1;
            return find(c, n->leftChild, currentDep);
        }
        if (n->value < c) {
            // Value is greater than current node. Go to node's right child.
            int currentDep = dep + 1;
            return find(c, n->rightChild, currentDep);
        }
...
public:
    ...
    int find(const Comparable &c) const {
        // calls private helper function, -1 is the default level
        int depDefault = 0;
        return find(c, root, depDefault);
    }
